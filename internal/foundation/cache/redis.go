...
func (r *RClient) Set(ctx context.Context, key string, value interface{}) error {
	conn, err := r.pool.GetContext(ctx)
	if err != nil {
		return err
	}
...
func (r *RClient) IncrBy(ctx context.Context, key string, step int64) (int64, error) {
	conn, err := r.pool.GetContext(ctx)
	if err != nil {
		return -1, err
	}
	...
	val, err := redis.Int64(conn.Do("INCRBY", key, step))
	if err != nil {
		return 0, err
	}
	return val, nil
